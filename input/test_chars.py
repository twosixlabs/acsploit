...
def test_constructor():
    cg = CharGenerator()
    assert cg.get_min_value() in cg.characters  # check if minimum and maximum characters are in
    assert cg.get_max_value() in cg.characters  # the character set


def test_get_random():
    cg = CharGenerator()
    assert cg.is_valid(cg.get_random())  # generate a random character and see if it is valid
    assert cg.get_random() in cg.characters  # generate a random character and test if in char set
